...
export async function verifyToken(token, secret) {
  ...
  try {
    if (!await crypto.subtle.verify('HMAC', await key(secret), unb64u(sig), enc.encode(body))) return null;
...
export default {
  async fetch(request, env) {
    ...
    // ── перевірка токена (пригодиться далі) ───────────────────────
